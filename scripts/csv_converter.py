...
def convert_all_books_to_unicode_accents(book_list, drop_variants=True):
    ...
    for book in list_of_all_books:
        ...
        converted_book = convert_book_accents(path=path, drop_variants=drop_variants, book=book)
...
def make_one_verse_one_line(book_text):
    ...
    separated_list = [item for item in separated_list if item!=""] # Removing the empty elements of the list
...
######## Converting all books
print(
    "CONVERTING ALL BOOKS (NO ACCENTS) - SAVING TO THE no-accents FOLDER"
)
